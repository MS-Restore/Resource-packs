...
- 删除了全部来自 `BlueArchive Halos` 的材质（不再对未安装对应模组玩家显示模型） - MT15
- 删除了一些未使用到的资源文件 - MT16
- 修改了一些翻译内容 - MT17

### a1.10.5 build.3 `2026/01/18`

- 此版本由 `HelveticaVolubi` 构建
- 再次修改了打开书的界面（将背景中的图文居中）---MT14(re-edit of MT13)

### a1.10.5 build.2 `2026/01/11`

- 此版本由 `HelveticaVolubi` 构建
- 移动了点击音效的位置---MT11
- 修改了标题和版本的图片---MT12
- 修改了打开的书的界面（图片来自Blue-Archive-JP官网）---MT13

### a1.10.5 build.1 `2026/01/07`

- 此版本由 `HelveticaVolubi` 构建
- 此版本进行了实验性的修改（可能回滚）
- 此版本将语言文件简化至仅保留修改的项目---MT9
- 更改了部分文字描述---MT10

### r1.10.4 repatched.3 `2026/01/07`

- 此版本由 `HelveticaVolubi` 构建
- 回滚了部分画材质的更改---MT8

### r1.10.4 repatched.2 `2026/01/07`

- 此版本由 `HelveticaVolubi` 构建
- 修改了 `zh_cn.json` 中全部的 `MC-Schale` 为 `MS-Restore` ---MT5
- 修改了全部材质文件中的 `MC-Schale` 为 `MS-Restore` ---MT6
- 移除了一些材质的修改（相关修改 - `RP20` `RP10` `RP5`）---MT7
...
- 此版本起不再保留对基岩版的兼容
- 此版本为重构版本，故不再使用以前的RP标记更新内容，使用MT标记
- 移除了全部地铁相关音频---MT1
- 移除了对部分生物的修改---MT2
- 更改了资源包的图标和简介---MT3
...
- 替换了 `records` 声音文件夹中 `mall.ogg` 的音乐为 `Irasshaimase` ---RP15
